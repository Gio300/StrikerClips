...
-- ========== 007. Tournament Stat Check, Submit Results (tournament_admins, tournament_results) ==========
...
create policy "Tournament owner add admins" on public.tournament_admins for insert with check (exists (select 1 from public.tournaments t where t.id = tournament_id and t.created_by = auth.uid()));
create policy "Tournament owner remove admins" on public.tournament_admins for delete using (exists (select 1 from public.tournaments t where t.id = tournament_id and t.created_by = auth.uid()));
create policy "Tournament owner admins update stat check" on public.stat_check_submissions for update using (tournament_id is not null and (exists (select 1 from public.tournaments t where t.id = tournament_id and t.created_by = auth.uid()) or exists (select 1 from public.tournament_admins ta where ta.tournament_id = stat_check_submissions.tournament_id and ta.user_id = auth.uid())));
create policy "Tournament results viewable" on public.tournament_results for select using (true);
create policy "Tournament owner admins insert results" on public.tournament_results for insert with check (exists (select 1 from public.tournaments t where t.id = tournament_id and t.created_by = auth.uid()) or exists (select 1 from public.tournament_admins ta where ta.tournament_id = tournament_id and ta.user_id = auth.uid()));
